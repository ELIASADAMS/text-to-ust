    ...
    for line_num, raw_line in enumerate(lines, 1):
        ...
        if line.startswith("[") and line.endswith("]") and len(line) > 2:
            ...
            if section_name:
                ...
                msg = f"вљ пёЏ Empty section '[]' on line {line_num} - using 'Main'"
                if on_warning:
                    on_warning(msg)
            continue
            ...
            try:
                ...
                msg = f"вљ пёЏ Parse error line {line_num}: '{line}' в†’ {e}"
                if on_warning:
                    on_warning(msg)
            continue
...
def text_to_ustx(
    ...
    for element in text_elements:
        ...
        if accent != "None" and romaji_phoneme not in ["гЃЈ", "+"]:
            if word_start or romaji_phoneme in [" ", "гЂЂ", "гЂЃ", "пјЊ"]:
                if word_phonemes:
...
class USTGeneratorApp:
    def __init__(self, root):
        ...
        input_frame = ttk.LabelFrame(
            root, text="рџЋµ Song Lyrics (Romaji/Hiragana/Katakana)", padding=12
        )
        ...
        self.lyrics_text.insert(
            "1.0",
            """[Verse 1]
гЃЌг‚ѓгЃЈгЃЌг‚ѓ гЃ†г‚ЊгЃ— гЃ„гЃџгЃ„ гЃ•гЃ¶г‚Љ
г‚†гЃігЃ•гЃЌ гЃЌг‚ЉгЃ•гЃ‘ гЃ‚гЃ‹гЃ„ гЃ¤г‚†

[Chorus]
гЃ„гЃџгЃї гЃ„гЃџгЃї гЃЌг‚‚гЃЎгЃ„гЃ„""",
        )
        ...
        # Panel 1: Timing (Left)
        timing_panel = ttk.LabelFrame(controls_main, text="вЏ±пёЏ Timing", padding=10)
        ...
        voice_panel = ttk.LabelFrame(
            controls_main, text="рџЋ¤ Voice & Length", padding=10
        )
        ...
        melody_panel = ttk.LabelFrame(controls_main, text="рџЋµ Melody Modes", padding=10)
        melody_panel.pack(side="left", fill="y", padx=(0, 8))

        self.motif_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(
            melody_panel, text="рџЋј Motif Memory", variable=self.motif_var
        ).pack(anchor="w", pady=2)

        self.lyrical_mode_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(
            melody_panel, text="рџЋ­ Lyrical Mode", variable=self.lyrical_mode_var
        ).pack(anchor="w", pady=2)

        self.flat_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(
            melody_panel, text="рџЋ№ Monotone (Flat)", variable=self.flat_var
        ).pack(anchor="w", pady=2)

        self.quartertone_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(
            melody_panel, text="в™Ї Microtones (Qt)", variable=self.quartertone_var
        ).pack(anchor="w", pady=2)

        self.chord_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(
            melody_panel, text="рџЋё I-IV-V Chords", variable=self.chord_var
        ).pack(anchor="w", pady=2)
        ...
        output_panel = ttk.LabelFrame(controls_main, text="вљ™пёЏ UST/Output", padding=6)
        ...
        ttk.Button(ust_frame, text="рџЋІ", width=3, command=self.randomize_seed).grid(
        ...
        if USTX_AVAILABLE:
            ...
            ttk.Checkbutton(output_panel, text="рџЊџ USTX Mode",
                            variable=self.ustx_mode_var).pack(anchor="w", pady=2)

        btn_frame = ttk.Frame(output_panel)
        btn_frame.pack(fill="x")
        ttk.Button(btn_frame, text="рџЋµ Gen", command=self.generate_ust).pack(
            fill="x", pady=1
        )
        ttk.Button(btn_frame, text="рџ’ѕ Save", command=self.save_ust_only).pack(
            fill="x", pady=1
        )
        ttk.Button(btn_frame, text="рџ“‹ Prev", command=self.preview_phonemes).pack(
            fill="x", pady=1
        )
        ttk.Button(btn_frame, text="рџ§№ Clear", command=self.clear).pack(
            fill="x", pady=1
        )

        ttk.Button(btn_frame, text="рџ’ѕ Preset", command=self.save_preset).pack(
            fill="x", pady=1
        )
        ttk.Button(btn_frame, text="рџ“‚ Load", command=self.load_preset).pack(
            fill="x", pady=1
        )
        ...
        self.status_var = tk.StringVar(value="вњ… Ready - All controls visible!")
        ...
        preview_frame = ttk.LabelFrame(root, text="рџ‘Ђ Preview", padding=8)
    ...
    def randomize_seed(self):
        ...
        self.status_var.set(f"рџЋІ New seed: {new_seed}")
    ...
    def _generate_content(self):
        ...
        if errors:
            self.status_var.set(f"вќЊ Fix: {' | '.join(errors)}")
            return None
        ...
        try:
            ...
            self.status_var.set(f"вњ… Parsed {len(elements)} elements вњ“")
            ...
            if USTX_AVAILABLE and self.ustx_mode_var.get():
                ...
                for element in elements:
                    ...
                    if self.accent_var.get() != "None" and romaji_phoneme not in ["гЃЈ", "+"]:
                        if word_start or romaji_phoneme in [" ", "гЂЂ", "гЂЃ", "пјЊ"]:
                            if word_phonemes:
                                word_length = len(word_phonemes)
            ...
            self.status_var.set(f"вљ пёЏ Rare error: {str(e)[:60]}")
            return None
    ...
    def generate_ust(self):
        ...
        try:
            with open(filename, "w", encoding="utf-8-sig") as f:
                f.write(ust_content)
            self.status_var.set(f"вњ… Saved {os.path.basename(filename)}!")

            self.preview_text.config(state="normal")
            self.preview_text.delete("1.0", tk.END)
            self.preview_text.insert("1.0", f"вњ… USTX Ready:\n\n{ust_content[:600]}...")
            self.preview_text.config(state="disabled")
        except Exception as e:
            self.status_var.set(f"вќЊ Save failed: {str(e)}")
    ...
    def save_ust_only(self):
        ...
        filename = filedialog.asksaveasfilename(
            defaultextension=".ust" if not (USTX_AVAILABLE and self.ustx_mode_var.get()) else ".ustx",
            initialfile=f"{self.project_var.get()}.ust" if not (
                        USTX_AVAILABLE and self.ustx_mode_var.get()) else f"{self.project_var.get()}.ustx",
            filetypes=[("UST files", "*.ust"), ("USTX files", "*.ustx"), ("All files", "*.*")],
    ...
    def preview_phonemes(self):
        lyrics = self.lyrics_text.get("1.0", tk.END).strip()
        if not lyrics:
            self.status_var.set("вќЊ No lyrics to preview")
            return
        ...
        preview = f"рџ”¤ {self.phoneme_mode_var.get()} Mode (first 30):\n\n"
        non_pause_count = 0
        ...
        for i, elem in enumerate(elements[:30]):
            if elem.startswith("PAUSE"):
                ...
                preview += f"{i:2d}: '{elem}' в†’ {hiragana}\n"
                non_pause_count += 1
        ...
        self.status_var.set(
            f"вњ… {self.phoneme_mode_var.get()}: {non_pause_count} phonemes"
        )
    ...
    def clear(self):
        ...
        self.status_var.set("рџ§№ Lyrics cleared вњ“")
    ...
    def save_preset(self):
        ...
        if filename:
            try:
                ...
                self.status_var.set(f"вњ… Preset saved: {os.path.basename(filename)}")
            except Exception as e:
                self.status_var.set(f"вќЊ Preset save failed: {str(e)[:50]}")
    ...
    def load_preset(self):
        ...
        try:
            ...
            self.status_var.set(f"вњ… Loaded: {os.path.basename(filename)}")
        except Exception as e:
            self.status_var.set(f"вќЊ Load failed: {str(e)[:50]}")
