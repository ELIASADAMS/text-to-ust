...
class USTXWriter:
    ...
    def add_small_tsu(self, root_key, length=60):
        self.notes.append({
            ...
            "tone": int(root_key),
            "lyric": "っ",
            "pitch": {"data": [{"x": 0, "y": 20, "shape": "io"}], "snap_first": True},
    ...
    def add_note(self, length, lyric, note_num, pre_utter, voice_overlap, intensity, envelope, pbs="0;0", pbw="0",
                 flags="", bpm=120):
        # BPM SNAP - Round lengths to beat grid
        ticks_per_beat = int(480 * 60 / bpm)  # 480 ticks/quarter @ 60BPM base
        snapped_length = round(length / ticks_per_beat) * ticks_per_beat
        snapped_length = max(60, min(snapped_length, length * 2))  # Keep reasonable bounds
        ...
        if pbs != "0;0" and ";" in pbs:
            try:
                ...
                if drop_strength < -20:  # Pitch drop detected
                    ...
                    pitch_data = [
                        {"x": -60, "y": drop_strength * 0.8, "shape": "io"},  # Sharp drop
                    ...
                    pitch_data = [{"x": -40, "y": drop_strength, "shape": "io"}, {"x": 40, "y": 0, "shape": "io"}]
            except:
                pass
        else:
        ...
        vibrato = {
            ...
            "in": 20, "out": 20, "shift": 0,
            "drift": random.randint(0, 57), "vol_link": 0
        }

        phoneme_expressions = [
            {"index": 0, "abbr": "mod", "value": 10},
            {"index": 0, "abbr": "vel", "value": 180},
            {"index": 0, "abbr": "vol", "value": intensity}
        ]

        self.notes.append({
            "position": self.position,
            "duration": snapped_length,  # ✅ BPM SNAPPED LENGTH
            "tone": int(round(note_num)),
            "lyric": lyric,
        ...
        self.position += snapped_length  # Use snapped length for positioning
