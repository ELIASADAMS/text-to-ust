...
def create_stretch_notes(
    phoneme: str, stretch_prob: float = 0.25, max_stretch: int = 3, brain=None
) -> list:
    """
        ...
        try:
            if getattr(sys, "frozen", False):
                ...
                icon_path = os.path.join(
                    os.path.dirname(os.path.dirname(script_dir)), "hibiki.ico"
                )
...
いたみ いたみ きもちいい""",
        ...
        if USTX_AVAILABLE:
            ttk.Button(
                btn_frame, text="🌟 Export USTX", command=self.save_ust_only
            ).pack(fill="x", pady=1)
    ...
    def _generate_content(self):
        ...
        try:
            ...
            logger.debug(
                "_generate_content: generated length -> %s",
                len(ust_content) if ust_content else None,
            )
            return ust_content
        except Exception as e:
    ...
    def generate_ust(self):
        ...
        if getattr(sys, "frozen", False):
            ...
            save_dir = os.path.dirname(os.path.dirname(current_dir))
        ...
        try:
            ...
            fd, tmp_path = tempfile.mkstemp(
                prefix="tmp_", suffix=ext, dir=save_dir, text=True
            )
            try:
                ...
                if os.path.exists(tmp_path) and not os.path.exists(filename):
                    try:
                        ...
                        pass
            logger.info(
                "Saved file: %s (bytes: %d)",
                filename,
                len(ust_content.encode("utf-8-sig")),
            )
    ...
    def save_ust_only(self):
        ...
        if getattr(sys, "frozen", False):
            ...
            current_dir = os.path.dirname(os.path.abspath(__file__))
            initial_dir = os.path.dirname(os.path.dirname(current_dir))
                ...
                try:
                    ...
                    if os.path.exists(tmp_path) and not os.path.exists(filename):
                        try:
                            ...
                            pass
                logger.info(
                    "Saved file via dialog: %s (bytes: %d)",
                    filename,
                    len(ust_content.encode("utf-8-sig")),
                )
                self.status_var.set(f"✅ Saved {os.path.basename(filename)}")
